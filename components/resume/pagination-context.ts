 ...
 *
 * The sheet stack works out its own page count, but §11.5's live indicator
 * belongs to the editor's chrome, outside the document — and in the editor
 * the resume lives in an iframe, so there is no DOM path between the two.
 * They are one React tree though (the preview is portalled), so a context
